...
% Translated by louisstuart96@gmail.com

\chapter{\protect\TeX\ 处理单元的结构}

本书以章为单位对 \TeX\ 各个方面的内容进行讨论，每一张都针对一个较小的、易于讲述的主题。
本章作为开篇，将给出 \TeX\ 工作方式的一个全貌。许多细节有必要在本章省略，而留给后续各章讲述。
但另一方面，本章给出的一些例子在后文合适的地方会重复出现，添加这些例子是为了保证本章内容的自洽。

\section{\protect\TeX\ 的四个处理单元}

\TeX\ 处理输入的方式可被看作在四个层次上进行。可以简单认为 \TeX\ 处理程序分为四个独立的单元，
每个单元接收前一步骤的输出，向下一个步骤呈递输入。
第一个步骤的输入来自 \n{.tex} 文件，最后一个步骤输出 \n{.dvi} 文件。

(For many purposes) 最为简单和直观的理解方式是将四个层次的处理看作是先后进行的，
每个层次\textbf{完整地}接收上一个层次的输出。事实上不完全是这样——所有层次同时工作，层次之间存在交互。

这四个层次（用 Knuth 的原话，可大致上分别类比为“眼睛”、“嘴巴”、“胃”和“肠”）分别为：

\begin{enumerate}
\item 输入层。这一部分 \TeX\ 的处理单元负责接收运行 \TeX\ 的电脑里的文件系统所呈递的输入，并将其转化为记号（token）。
记号是 \TeX\ 的内部对象：有直接用来排版文字的字符记号，也有作为命令被接下来两层处理的控制序列（control sequence）记号。
\item 展开层。在上一层产生的某一些（非全部）记号——宏、条件式以及 \TeX\ 的一些原始命令需要展开。
所谓展开，就是将记号或记号序列替换为其它记号的过程。
\item 执行层。不可展开的控制序列是可执行的，执行这些记号的过程发生在 \TeX\ 处理程序的第三层。
本层的一部分活动涉及 \TeX\ 内部状态的变化，属于这类变化的典型活动为变量的赋值（包括宏的定义）。
另外一部分主要的活动为水平列表、垂直列表和数学列表的构建。
\item 可视层。最后一层执行的是 \TeX\ 处理可视化内容的部分。
在此，水平列表被划分为段落，垂直列表被划分为页面，数学列表被构造成公式。同时 \n{dvi} 文件的生成也位于这一层。
这一层的算法对于用户而言不可见，但能够受到一些参数的影响。
\end{enumerate}

\section{输入处理单元}

\TeX\ 的输入处理单元是 \TeX\ 中负责把从输入文件中接收的字符翻译成记号的部分。
其输出为一个记号“流”，或称为记号列表。多数记号可归为前文提到两大类——字符记号和控制序列记号。
其余为参数记号，本章不作讨论。

\subsection{字符的输入}

对于简单的输入文本，字符将转为字符记号。然而，\TeX\ 会忽略一些输入记号，
比如一段连续的空格通常等效为一个空格。同时，\TeX\ 本身也会添加不对应输入文本里任何字符的记号，
例如行末插入的空格，或是空行后插入的 \cs{par} 记号。

并非所有字符记号都代表可排版的字符。在 \TeX\ 中字符被分为十六个类别，每个类别代表字符拥有的某类功能，
其中只有两类包含可排版的字符。其它类别包含诸如~\n{\char`\{}、\n{\char`\}}、\n\& 以及~\n\# 等字符。
一个字符记号可视为一对数字——字符码（通常为 ASCII 编码）和类别码。一个字符码所对应的类别码是可修改的。

当转义字符（默认为 \cs{}）出现在输入文本中时，\TeX\ 构建记号的行为更加复杂。
简单来说，\TeX\ 将从输入文本中读取若干字符合并为一个记号，从而构造一个控制序列。

\TeX\ 的输入处理单元对类别码的响应行为可以描述成一个在三种状态中切换的状态机：状态 $N$（一行开头，new line）、
状态 $M$（一行中间，middle of line）、状态 $S$（跳过空格，skipping spaces）。三种状态和互相之间的转换将在第~\ref{mouth} 章讨论。

\subsection{对输入的两步处理}

\TeX\ 的输入处理单元实际上本身包括两个层次的处理。由于输入终端、编辑器或操作系统的限制，
用户可能无法直接输入某些需要的字符。因此，\TeX\ 提供了一种机制，通过两个上标符号访问到所有可用的字符码位。
这可看作一个单独的处理步骤，先于上文所提到的三态状态机。

例如，字串 \verb>^^+> 被替换为 \n{k}，因为 \n{k} 和 \n{+} 的 ASCII 码相差 64。
由于这个处理步骤在构造记号之前进行，\verb>\vs^^+ip> 的写法和 \verb>\vskip 5cm> 完全等效。
比这个更有用的例子也有。

需要注意，两步处理的第一个步骤只进行字符到字符的转换，不涉及到字符的类别码。
后者在输入处理的第二个步骤才会起作用，该步骤通过将字符码与其类别码结合转换为字符记号。

\section{展开处理单元}

\TeX\ 的展开处理单元接受记号流，对其中的记号逐个展开，直到只剩下不可展开的记号。
最有代表性的例子是对宏的展开：如果一个控制序列是一个宏名，它将会（和参数记号一起，如果有的话）被替换为这个宏的定义文本。

展开处理单元的输入主要由输入处理单元提供。来自 \TeX\ 第一个处理步骤的记号流传递到此进行展开，
结果为不可展开的记号流，呈递给执行处理单元。

然而，展开处理单元也会在（其他处理单元）处理 \cs{edef} 或者 \cs{write} 等命令时起作用。
作为命令参数的记号列表被充分展开，好比它们是来自上一个步骤的记号列表，而不是某个命令的参量。
...
\begin{enumerate}
\item 首先看记号是否可以展开。
\item 如果记号不可展开，直接放入当前正在构建的（不可展开的）记号列表，然后读入下一个记号。
\item 反之将记号替换成其展开结果。对于不带参数的宏以及少数简单的原始命令如 \cs{jobname} 等，
      这就是一个简单的替换；而通常情况下 \TeX\ 需要从记号流里获取若干记号作为参量，以构成当前记号的替换文本。
      例如，如果当前记号为一个带参数的宏，则需要获取足够多的记号构建与宏的参数对应的参量。
\item 从展开结果的第一个记号起继续展开过程。
\end{enumerate}

判断一个记号是否可以展开很简单。宏、活动字符、条件式，以及一部分 \TeX\ 原始命令
（见第 \pageref{expand:lijst} 页的列表）是可以展开的，其它的不能。
于是展开处理单元将宏替换为其展开结果，计算条件式的结果并消除与条件无关的部分，
而类似 \cs{vskip} 这样的记号，以及字符记号，包括特殊字符记号如美元符和括号等，则原封不动。

\subsection{特殊情形：\cs{expandafter}，\cs{noexpand} 和 \cs{the}}

上文所述，在一个记号被展开后，\TeX\ 将开始展开结果中的记号。
第一眼看上去，\cs{expandafter} 命令好像不符合这个规则，因为它只展开一步。实际上发生的是，整个序列
\begin{disp}
\cs{expandafter}\gr{token$_1$}\gr{token$_2$}
\end{disp}
被替换为
\begin{disp}
\gr{token$_1$}\gr{\itshape expansion of token$_2$}
\end{disp}
接着替换结果重新被展开处理单元检查。

然而实际的例外还是有的。如果当前的记号为 \cs{noexpand} 命令，那么下一个记号此时被视为不可展开的：
它在处理时被视为等同于 \cs{relax}，被直接放入正在构建的不可展记号列表。
...
替换文本 \verb>\noexpand\b> 在定义时展开。\cs{noexpand} 的展开结果为下一个记号，
其意义临时等同于 \cs{relax}。因此，展开处理单元处理下一个记号 \cs{b} 时，将之视为不可展开的记号，
传递给正在构建的记号列表，亦即宏 \cs{a} 的替换文本。

另一个例外是，如果在 \cs{edef} 宏定义中的记号来自记号列表变量的结果 \cs{the}\gr{token variable}，
那么这些记号也不会被进一步展开。
